...
class _LocalAndWebObjectsWidgetState extends State<LocalAndWebObjectsWidget> {
  ...
  double newRotationAmount = 0.0;


  void initState() {
    super.initState();
    timer = Timer.periodic(Duration(milliseconds: 100), (Timer t) => reloadAR());
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text('Демо динозавр'),
        ),
        body: Container(
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                            ...
                            child: Text("Додати/видалити динозавра")),
                      ],
                    ),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                      children: [
                        ElevatedButton(
                            ...
                            child: Text("Змінити розташування")),
                      ],
                    ),
                  ]))
            ])));
  ...
  Future<void> onLocalObjectAtOriginButtonPressed() async {
    if (this.localObjectNode != null) {
      ...
      var newNode = ARNode(
          ...
          uri: "Models/t_rex/scene.gltf",
          scale: Vector3(0.1, 0.1, 0.1),
          position: Vector3(0.0, 0.0, -0.4),
          rotation: Vector4(-0.7, 0.5, -0.5, -0.5));
  ...
  Future<void> reloadAR() async {
    if (this.localObjectNode != null) {
     ...
     // var newTranslationAxis = Random().nextInt(3);
      ...
      var newRotationAxisIndex = Random().nextInt(3);
      var newRotationAmount = Random().nextDouble();
      var newRotationAxis = Vector3(0.0, 1.0, 0);
      //newRotationAxis[newRotationAxisIndex] = 1.0;

      final newTransform = Matrix4.identity();

    newTransform.setTranslation(Vector3(0.0, 0.0, -4.4));
    newTransform.rotate(newRotationAxis, this.newRotationAmount+=0.01);
    // newTransform.scale(newScale);
